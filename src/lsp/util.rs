...
impl Document {
    pub fn new(content: String, tree: Tree, parser: Arc<dyn LspParser>) -> Self {
        Self {
            content,
            tree,
            parser,
    ...
    pub fn find_node_at(&self, line: usize, utf16_col: usize) -> Option<tree_sitter::Node<'_>> {
    ...
    pub fn utf16_col_to_byte_col(&self, line: usize, utf16_col: usize) -> usize {
        let line_content = self.content.lines().nth(line).unwrap_or("");
    ...
    pub fn extract_symbols(&self) -> Vec<crate::parser::DocumentSymbol> {
        self.parser.extract_symbols(&self.tree, &self.content)
    }

    pub fn resolve_symbol(&self, line: usize, utf16_col: usize) -> Option<crate::parser::SymbolResolution> {
        let byte_col = self.utf16_col_to_byte_col(line, utf16_col);
        self.parser.resolve_symbol(&self.tree, &self.content, line, byte_col)
    }
}
