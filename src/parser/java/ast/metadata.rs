...
use crate::model::lang::java::{JavaElement, JavaParameter};
...
impl JavaParser {
                ...
                if let Some(parent_node) = self.find_next_enclosing_definition(meta_cap.node) {
                    if let Some(parent_name_node) = parent_node.child_by_field_name("name") {
                        ...
                        if let Some(&idx) = entities_map.get(&enclosing_fqn) {
                            self.attach_metadata_to_model(captures, source, enclosing_fqn, &mut entities[idx].element, relations);
                        }
                    }
                }
            }
    ...
    fn attach_metadata_to_model<'a>(
        ...
        if let Some(mods_node) = captures.iter().find(|c| c.index == self.indices.mods).map(|c| c.node) {
        ...
        match element {
            JavaElement::Class(_) => {
                if let Some(s) = captures.iter().find(|c| c.index == self.indices.class_super) {
                    ...
                    relations.push(JavaRelation {
                        source_fqn: fqn.clone(), target_name: s_name,
                        rel_type: EdgeType::InheritsFrom, range: None,
                    });
                }
                for cc in captures.iter().filter(|c| c.index == self.indices.class_inter) {
                    let i = cc.node.utf8_text(source.as_bytes()).unwrap_or_default().to_string();
                    relations.push(JavaRelation {
                        source_fqn: fqn.clone(), target_name: i,
                        rel_type: EdgeType::Implements, range: None,
                    });
                }
            }
            JavaElement::Interface(_) => {
                for cc in captures.iter().filter(|c| c.index == self.indices.inter_ext) {
                    let e = cc.node.utf8_text(source.as_bytes()).unwrap_or_default().to_string();
                    relations.push(JavaRelation {
                        source_fqn: fqn.clone(), target_name: e,
                        rel_type: EdgeType::InheritsFrom, range: None,
                    });
                }
            }
            JavaElement::Enum(_) => {
                for cc in captures.iter().filter(|c| c.index == self.indices.enum_interface) {
                    let i = cc.node.utf8_text(source.as_bytes()).unwrap_or_default().to_string();
                    relations.push(JavaRelation {
                        source_fqn: fqn.clone(), target_name: i,
                        rel_type: EdgeType::Implements, range: None,
            ...
            JavaElement::Method(m) => {
                ...
                if let (Some(t_node), Some(n_node)) = (
                    captures.iter().find(|c| c.index == self.indices.param_type).map(|c| c.node),
                    captures.iter().find(|c| c.index == self.indices.param_name).map(|c| c.node),
                ) {
    ...
    fn add_modifier(&self, element: &mut JavaElement, m_str: String) {
        match element {
            ...
            JavaElement::Method(m) => if !m.modifiers.contains(&m_str) { m.modifiers.push(m_str); }
