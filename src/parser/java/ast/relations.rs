    ...
    pub(super) fn resolve_relations<'a>(
        ...
        for captures in all_matches {
            if let Some(call_cap) = captures.iter().find(|c| c.index == self.indices.call) {
                if let Some(target_node) = captures
                    .iter()
                    .find(|c| c.index == self.indices.call_name)
                    .map(|c| c.node)
                {
                ...
                if let Some(target_node) = captures
                    .iter()
                    .find(|c| c.index == self.indices.inst_type)
                    .map(|c| c.node)
                {
            ...
            } else if let Some(fa_cap) = captures
                .iter()
                .find(|c| c.index == self.indices.field_access_meta)
            {
                if let Some(target_node) = captures
                    .iter()
                    .find(|c| c.index == self.indices.field_name_node)
                    .map(|c| c.node)
                {
                ...
                if let Some(gp) = parent.parent() {
                    if gp.kind() == "field_declaration" {
                        if let Some(name_node) = parent.child_by_field_name("name") {
                            return self.get_fqn_for_definition(
                                &name_node,
                                source,
                                package.as_deref(),
                            );
