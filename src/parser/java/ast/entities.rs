    ...
    pub(crate) fn identify_entities<'a>(
        ...
        for captures in all_matches {
            ...
            if let Some(anchor) = definition_anchor {
                ...
                if let Some(name_node) = captures.iter().find(|c| c.index == name_idx).map(|c| c.node) {
                    ...
                    if !entities_map.contains_key(&fqn) {
                        ...
                        let element = self.create_java_element(kind, &fqn, &name, range, name_range, captures, source, relations);
                        entities.push(JavaEntity { element, node: anchor_node });
    ...
    fn create_java_element<'a>(
        ...
        match kind {
            KIND_LABEL_CLASS => JavaElement::Class(JavaClass {
                id: fqn.to_string(), name: name.to_string(), modifiers: vec![],
            ...
            KIND_LABEL_METHOD | KIND_LABEL_CONSTRUCTOR => {
                ...
                if let Some(ret_node) = captures.iter().find(|c| c.index == self.indices.method_ret).map(|c| c.node) {
                ...
                JavaElement::Method(JavaMethod {
                    id: fqn.to_string(), name: name.to_string(), return_type,
                    parameters: vec![], modifiers: vec![], is_constructor: kind == KIND_LABEL_CONSTRUCTOR,
                    range: Some(range), name_range: Some(name_range),
                })
            },
            KIND_LABEL_FIELD => {
                let anchor_node = captures.iter().find(|c| c.index == self.indices.field_def).unwrap().node;
                let type_node = captures.iter()
                ...
                JavaElement::Field(JavaField {
                    id: fqn.to_string(), name: name.to_string(), type_ref,
