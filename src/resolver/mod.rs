...
pub trait SemanticResolver: Send + Sync {
    ...
    fn resolve_type_of(&self, index: &CodeGraph, res: &SymbolResolution) -> Vec<SymbolResolution>;

    /// Find implementations or overrides of a symbol (global view)
    fn find_implementations(&self, index: &CodeGraph, res: &SymbolResolution) -> Vec<NodeIndex>;
}
