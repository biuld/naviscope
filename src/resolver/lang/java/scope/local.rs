...
impl SemanticScope<ResolutionContext<'_>> for LocalScope<'_> {
    fn resolve(&self, name: &str, context: &ResolutionContext) -> Option<Result<SymbolResolution, ()>> {
        ...
        self.parser.find_local_declaration(context.node, name, context.source)
...
mod tests {
    ...
    fn test_local_scope_resolve() {
        ...
        let x_node = tree.root_node().named_descendant_for_point_range(
        ...
        let scope = LocalScope { parser: &java_parser };
