...
async fn handle_socket(socket: WebSocket, mcp: McpServer) {
    ...
    let mut ws_to_mcp = tokio::spawn(async move {
        while let Some(msg_result) = StreamExt::next(&mut ws_stream).await {
            let msg_result: Result<Message, axum::Error> = msg_result;
            match msg_result {
                Ok(msg) => match msg {
                    Message::Binary(data) => {
    ...
    let mut mcp_to_ws = tokio::spawn(async move {
        ...
        while let Ok(n) = client_reader.read(&mut buf).await {
            ...
            let send_res: Result<(), axum::Error> =
                SinkExt::send(&mut ws_sink, Message::Binary(buf[..n].to_vec().into())).await;
            if send_res.is_err() {
                break;
            }
        }
