...
pub fn load_query(
    language: &Language,
    scm: &str,
) -> Result<Query, Box<dyn std::error::Error + Send + Sync>> {
    Query::new(language, scm).map_err(|e| format!("Invalid query: {:?}", e).into())
}

/// Gets the index of a capture name in a query.
pub fn get_capture_index(
    query: &Query,
    name: &str,
) -> Result<u32, Box<dyn std::error::Error + Send + Sync>> {
    query
        .capture_index_for_name(name)
        .ok_or_else(|| format!("Capture name '{}' not found in SCM", name).into())
}
...
pub fn build_symbol_hierarchy(raw_symbols: Vec<RawSymbol>) -> Vec<DisplayGraphNode> {
    use std::collections::HashMap;
    let mut symbols_map: HashMap<usize, usize> = HashMap::new(); // node_id -> flat_index
    let mut flat_symbols: Vec<DisplayGraphNode> = Vec::new();
    ...
    for (i, raw) in raw_symbols.iter().enumerate() {
        flat_symbols.push(DisplayGraphNode {
            id: raw.name.clone(),
            name: raw.name.clone(),
            kind: raw.kind.clone(),
            lang: String::new(),
...
macro_rules! decl_indices {
    ($name:ident, { $($field:ident => $capture:expr),+ $(,)? }) => {
        ...
        impl $name {
            pub fn new(query: &tree_sitter::Query) -> std::result::Result<Self, Box<dyn std::error::Error + Send + Sync>> {
                Ok(Self {
                    $($field: $crate::utils::get_capture_index(query, $capture)?,)+
                })
            }
        }
    };
...
/// Converts a (line, col) position to a byte offset in the content.
